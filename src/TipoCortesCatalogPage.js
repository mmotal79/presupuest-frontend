...
// Fecha de Creación: 31/07/2025 23:19
// Nombre del Archivo: TipoCortesCatalogPage.js (Control de cambio y secuencia N° 001: Implementación de CRUD para Catálogo de Tipos de Corte)
...
/**
 * Componente para gestionar el catálogo de Tipos de Corte.
 ...
 */
const TipoCortesCatalogPage = ({ userToken }) => {
  // URL base de la API para este catálogo
  const apiBaseUrl = "http://localhost:3001/api/catalogos/tipos-corte";
  const catalogTitle = "Catálogo de Tipos de Corte";
   ...
   */
  const fetchTiposCorte = useCallback(async () => {
    if (!userToken) {
  ...
  const handleOpenEditModal = (item) => {
    ...
    setFormData({
      ...
      factorConsumoTela: item.factorConsumoTela || 0,
      factorTiempoConfeccion: item.factorTiempoConfeccion || 0,
  ...
  const confirmDeleteItem = async () => {
    ...
    try {
      ...
      fetchTiposCorte(); // Actualizar la lista
    } catch (err) {
  ...
  const handleSaveItem = async (e) => {
    ...
    try {
      ...
      fetchTiposCorte(); // Recargar la lista
      handleCloseModal();
    } catch (err) {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        ...
        >
          ...
          <span>Añadir Tipo de Corte</span>
        </button>
      </div>
      ...
      {tiposCorte.length > 0 ? (
            ...
            <thead className="bg-gray-100">
              <tr>
                ...
                <th
                  ...
                  className="px-6 py-3 text-left text-xs font-medium text-gray-800 uppercase tracking-wider"
                >
                  Factor de Consumo de Tela
                </th>
                <th
                  scope="col"
                  className="px-6 py-3 text-left text-xs font-medium text-gray-800 uppercase tracking-wider"
                >
                  Factor de Tiempo de Confección
                </th>
                <th
                  scope="col"
                  className="px-6 py-3 text-left text-xs font-medium text-gray-800 uppercase tracking-wider"
              ...
              {tiposCorte.map((item) => (
                <tr key={item._id || uuidv4()} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.factorConsumoTela}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.factorTiempoConfeccion}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        ...
        <div className="text-center p-8 bg-white rounded-lg shadow-md">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Aún no has agregado ningún tipo de corte. Empieza haciendo clic en el botón de arriba.
          </p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-40 overflow-y-auto bg-gray-600 bg-opacity-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 relative">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-4">
              {isEditMode ? 'Editar Tipo de Corte' : 'Nuevo Tipo de Corte'}
            </h3>
            <form onSubmit={handleSaveItem}>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Factor de Consumo de Tela</label>
                    <input
                      type="number"
                      name="factorConsumoTela"
                      value={formData.factorConsumoTela}
                      onChange={handleInputChange}
                      className="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                      step="0.01"
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Factor de Tiempo de Confección</label>
                    <input
                      type="number"
                      name="factorTiempoConfeccion"
                      value={formData.factorTiempoConfeccion}
                      onChange={handleInputChange}
                      className="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                      step="0.01"
                      required
        ...
        <div className="fixed inset-0 z-40 overflow-y-auto bg-gray-600 bg-opacity-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 text-center">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Estás a punto de eliminar el ítem: <span className="font-semibold">{currentItem?.tipo}</span>. Esta acción no se puede deshacer.
            </p>
...
export default TipoCortesCatalogPage;
