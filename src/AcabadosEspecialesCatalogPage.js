...
// Nombre del Archivo: AcabadosEspecialesCatalogPage.js (Control de cambio y secuencia N° 001: Implementación de CRUD para Catálogo de Acabados Especiales)
...
/**
 * Componente para gestionar el catálogo de Acabados Especiales.
 ...
 */
const AcabadosEspecialesCatalogPage = ({ userToken }) => {
  // URL base de la API para este catálogo
  const apiBaseUrl = "http://localhost:3001/api/catalogos/acabados-especiales";
  const catalogTitle = "Catálogo de Acabados Especiales";

  // Estados para la gestión de datos y UI
  const [acabadosEspeciales, setAcabadosEspeciales] = useState([]);
   ...
   */
  const fetchAcabadosEspeciales = useCallback(async () => {
    if (!userToken) {
    ...
    try {
      ...
      setAcabadosEspeciales(data);
    } catch (err) {
      console.error("Error fetching acabados especiales:", err);
  ...
  const handleOpenEditModal = (item) => {
    ...
    setFormData({
      ...
      costoPorUnidad: item.costoPorUnidad || 0,
    });
    setCurrentItem(item);
    setIsModalOpen(true);
  };
  ...
  const confirmDeleteItem = async () => {
    ...
    try {
      ...
      fetchAcabadosEspeciales(); // Actualizar la lista
    } catch (err) {
  ...
  const handleSaveItem = async (e) => {
    ...
    try {
      ...
      fetchAcabadosEspeciales(); // Recargar la lista
      handleCloseModal();
    } catch (err) {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {acabadosEspeciales.length > 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Costo por Unidad ($)</th>
                <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Acciones</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {acabadosEspeciales.map((item) => (
                <tr key={item._id || uuidv4()} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.descripcion || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.costoPorUnidad?.toFixed(2) || 'N/A'}</td>
        ...
        <div className="text-center p-8 bg-white rounded-lg shadow-md">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No hay acabados especiales en el catálogo</h3>
          <p className="mt-1 text-sm text-gray-500">
            Aún no has agregado ningún acabado. Empieza haciendo clic en el botón de arriba.
          </p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-40 overflow-y-auto bg-gray-600 bg-opacity-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 relative">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-4">
              {isEditMode ? 'Editar Acabado Especial' : 'Nuevo Acabado Especial'}
            </h3>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                >
                  {loadingModal ? (
                    ...
                    isEditMode ? 'Guardar Cambios' : 'Crear Acabado'
                  )}
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-40 overflow-y-auto bg-gray-600 bg-opacity-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 text-center">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Estás a punto de eliminar el acabado: <span className="font-semibold">{currentItem?.nombre}</span>. Esta acción no se puede deshacer.
            </p>
...
export default AcabadosEspecialesCatalogPage;
